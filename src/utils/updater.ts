...
export async function checkForUpdates() {
    changes = await Unwrap(SwancordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(SwancordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(SwancordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(SwancordNative.updater.getRepo());
