...
import { SWANCORD_USER_AGENT } from "@shared/swancordUserAgent";
...
import { serializeErrors, SWANCORD_FILES } from "./common";
...
async function githubGet<T = any>(endpoint: string) {
    return fetchJson<T>(API_BASE + endpoint, {
        headers: {
            ...
            "User-Agent": SWANCORD_USER_AGENT
        }
    });
}
...
async function fetchUpdates() {
    ...
    data.assets.forEach(({ name, browser_download_url }) => {
        if (SWANCORD_FILES.some(s => name.startsWith(s))) {
