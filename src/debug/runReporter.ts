...
async function runReporter() {
    ...
    try {
        ...
        addPatch({
            ...
            replacement: {
                match: /(?<="use strict";)/,
                replace: "Swancord.Webpack._initReporter();"
            }
        }, "Swancord Reporter");

        // @ts-expect-error
        Swancord.Webpack._initReporter = function () {
        ...
        for (const [searchType, args] of Webpack.lazyWebpackSearchHistory) {
            ...
            try {
                ...
                if (result == null || (result.$$swancordGetWrappedComponent != null && result.$$swancordGetWrappedComponent() == null)) throw new Error("Webpack Find Fail");
            } catch (e) {
                let logMessage = searchType;
                if (method === "find" || method === "proxyLazyWebpack" || method === "LazyComponentWebpack") {
                    if (args[0].$$swancordProps != null) {
                        logMessage += `(${args[0].$$swancordProps.map(arg => `"${arg}"`).join(", ")})`;
                    } else {
...
// Run after the Swancord object has been created.
// We need to add extra properties to it, and it is only created after all of Swancord code has ran
