...
function SwancordPopoutButton({ buttonClass }: { buttonClass: string; }) {
        ...
        >
            {(_, { isShown }) => (
                <HeaderBarIcon
                    ...
                    onClick={() => setShow(v => !v)}
                    tooltip={isShown ? null : "Swancord Toolbox"}
...
export default definePlugin({
    name: "SwancordToolbox",
    description: "Adds a button to the titlebar that houses Swancord quick actions",
    ...
    TrailingWrapper({ children, className }: PropsWithChildren<{ className: string; }>) {
        return (
            <>
                {children}
                <ErrorBoundary noop>
                    <SwancordPopoutButton buttonClass={className} />
                </ErrorBoundary>
            </>
        );
    },
