...
export async function importSettings(data: string) {
    ...
    if ("settings" in parsed && "quickCss" in parsed) {
        ...
        await SwancordNative.settings.set(parsed.settings);
        await SwancordNative.quickCss.set(parsed.quickCss);
    } else
        throw new Error("Invalid Settings. Is this even a Swancord Settings file?");
}

export async function exportSettings({ minify }: { minify?: boolean; } = {}) {
    const settings = SwancordNative.settings.get();
    const quickCss = await SwancordNative.quickCss.get();
    return JSON.stringify({ settings, quickCss }, null, minify ? undefined : 4);
}

export async function downloadSettingsBackup() {
    const filename = `swancord-settings-backup-${moment().format("YYYY-MM-DD")}.json`;
...
export async function uploadSettingsBackup(showToast = true): Promise<void> {
    if (IS_DISCORD_DESKTOP) {
        const [file] = await DiscordNative.fileManager.openFiles({
            filters: [
                { name: "Swancord Settings Backup", extensions: ["json"] },
                { name: "all", extensions: ["*"] }
            ]
        });
