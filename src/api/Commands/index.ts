...
import { CommandArgument, CommandContext, CommandOption } from "@swancord/discord-types";

import { sendBotMessage } from "./commandHelpers";
import { ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType, SwancordCommand } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: SwancordCommand[];
export const commands = {} as Record<string, SwancordCommand>;
...
export const _init = function (cmds: SwancordCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: SwancordCommand, args: CommandArgument[], ctx: CommandContext) {
    if (!cmd.isSwancordCommand)
 ...
 * @param opt
 */
export function prepareOption<O extends CommandOption | SwancordCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: SwancordCommand, plugin: string) {
...
export function registerCommand<C extends SwancordCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",
    ...
    command.isSwancordCommand = true;
