...
While it was primarily created for Swancord, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @swancord/discord-types
yarn add -D @swancord/discord-types
pnpm add -D @swancord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@swancord/discord-types";
...
```ts
import { ApplicationCommandType } from "@swancord/discord-types/enums";
