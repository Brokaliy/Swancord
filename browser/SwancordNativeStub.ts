...
// Only import pure modules that don't import other parts of Swancord.
...
const setCssDebounced = debounce((css: string) => SwancordNative.quickCss.set(css));

const themeStore = DataStore.createStore("SwancordThemes", "SwancordThemeData");

// probably should make this less cursed at some point
window.SwancordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Vendicated/Swancord" }),
    ...
    quickCss: {
        get: () => DataStore.get("SwancordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("SwancordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "SwancordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => SwancordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("SwancordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("SwancordSettings", JSON.stringify(s)),
